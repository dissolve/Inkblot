...
class ModelContactsContact extends Model {
    ...
    public function getContact($contact_id) {
        $results = $this->db->query("SELECT * FROM ".DATABASE.".contacts WHERE contact_id = ".(int)$contact_id);
        return $results->row;
    }

    public function getContactGroups($contact_id) {
        $results = $this->db->query("SELECT groups.* FROM ".DATABASE.".groups JOIN ".DATABASE.".frined_group USING(group_id) WHERE contact_id = ".(int)$contact_id);
        return $results->rows;
    }

    public function getContactData($contact_id) {
        $results = $this->db->query("SELECT * FROM ".DATABASE.".contact_field JOIN ".DATABASE.".field_types USING(field_type_id) WHERE contact_id = ".(int)$contact_id);
        return $results->rows;
    }
    ...
    public function addDataToContact($contact_id, $field_type_id, $field_value) {
        $this->db->query("INSERT INTO ".DATABASE.".contact_field 
    ...
    public function getTotalContacts(){
        $query = $this->db->query("SELECT count(contact_id) as total FROM " . DATABASE . ".contacts");
    ...
    public function getContacts($limit=20, $skip=0){
        ...
        $query = $this->db->query("SELECT contact_id FROM " . DATABASE . ".contacts WHERE contact_id != 0 LIMIT ". (int)$skip . ", " . (int)$limit);
        $data = $query->rows;
        $data_array = array();
