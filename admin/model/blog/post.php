	...
	public function getPost($post_id) {
        ...
        $post['permalink'] = preg_replace( '`/admin/`', '/',
            rewrite_url($this->url->link('blog/post', 'year='.$post['year']. '&' . 
                                        ...
                                        'slug=' . $post['slug'], '')), 1);
        //$post['shortlink'] = $this->short_url->link('blog/shortener', 'eid='.$this->num_to_sxg($post['post_id']), '');
        $post['shortlink'] = $this->config->get('config_short_url').'s/'.$this->num_to_sxg($post['post_id']);
