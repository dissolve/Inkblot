...
<div class="h-entry hentry">
  ...
  <?php foreach($post['context'] as $ctx){ ?>
    ...
    <?php } ?>
          ...
          <article id="photo-<?php echo $post['post_id']?>" class="photo-<?php echo $post['post_id']?> photo type-photo status-publish format-standard category-uncategorized <?php echo ($post['draft'] == 1 ? 'draft':'') ?> <?php echo ($post['deleted'] == 1 ? 'deleted':'') ?>">
