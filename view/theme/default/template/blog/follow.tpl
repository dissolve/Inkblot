...
<div>
  ...
  <?php foreach($post['context'] as $ctx){ ?>
    ...
    <?php } ?>
          ...
          <article id="follow-<?php echo $post['post_id']?>" class="follow-<?php echo $post['post_id']?> follow type-follow status-publish format-standard category-uncategorized <?php echo ($post['draft'] == 1 ? 'draft':'') ?> <?php echo ($post['deleted'] == 1 ? 'deleted':'') ?>">
  ...
  <div class='articlebody'>
    ...
    <?php } ?>
      <div class="entry-content e-content follow <?php echo (empty($post['title']) ? 'p-name' : '')?>">
      <?php 
