...
<div class="h-entry hentry">
  ...
  <?php foreach($post['context'] as $ctx){ ?>
    ...
    <?php } ?>
          ...
          <article id="checkin-<?php echo $post['post_id']?>" class="checkin-<?php echo $post['post_id']?> checkin type-checkin status-publish format-standard category-uncategorized <?php echo ($post['draft'] == 1 ? 'draft':'') ?> <?php echo ($post['deleted'] == 1 ? 'deleted':'') ?>">
