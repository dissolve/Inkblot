...
<div>
  ...
  <?php foreach($post['context'] as $ctx){ ?>
    ...
    <?php } ?>
          ...
          <article id="post-<?php echo $post['post_id']?>" class="<?php echo $post['post_type']?> type-<?php echo $post['post_type']?> <?php echo ($post['draft'] == 1 ? 'draft':'') ?> <?php echo ($post['deleted'] == 1 ? 'deleted':'') ?>">
