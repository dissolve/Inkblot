...
class ControllerPeopleView extends Controller {
    public function index()
    {
        ...
        if (!$this->session->data['is_owner']){
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/people/view')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/people/view', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/people/view', $data));
            }
        }
    }
