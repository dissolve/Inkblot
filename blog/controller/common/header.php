...
class ControllerCommonHeader extends Controller {
    ...
    public function clean()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/header_clean')) {
            return $this->load->view($this->config->get('config_template') . '/template/common/header_clean', $data);
        } else {
            return $this->load->view('default/template/common/header_clean', $data);
        }
    }
    ...
    public function contacts()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/header_contacts')) {
            return $this->load->view($this->config->get('config_template') . '/template/common/header_contacts', $data);
        } else {
            return $this->load->view('default/template/common/header_contacts', $data);
        }
    }
    ...
    private function commonSetup()
    {
        ...
        if(isset($this->request->server['HTTP_REFERER'])){
            ...
            $this->model_webmention_vouch->recordReferer($this->request->server['HTTP_REFERER']);
        }
        ...
        if (!$token 
                && isset($this->request->server['REDIRECT_HTTP_AUTHORIZATION'])
                && !empty($this->request->server['REDIRECT_HTTP_AUTHORIZATION'])){
            $parts = explode(' ', $this->request->server['REDIRECT_HTTP_AUTHORIZATION']);
            $token = $parts[1];
        }
        ...
        if(defined('STREAM_SERVICE_URL')){
            $this->response->addHeader('Link: <' . STREAM_SERVICE_URL . urlencode( $server . $this->request->server['REQUEST_URI']) . '>; rel="alternate"; type="application/activity+json"', false);
            $data['json_alternate'] = STREAM_SERVICE_URL . urlencode($server . $this->request->server['REQUEST_URI']);
        }
