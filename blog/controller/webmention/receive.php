...
class ControllerWebmentionReceive extends Controller {
    ...
    private function webmentionManager()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/webmention/list')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/webmention/list', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/webmention/list', $data));
        }
