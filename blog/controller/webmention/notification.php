...
class ControllerWebmentionNotification extends Controller {
    public function manifest()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/webmention/webapp_manifest')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/webmention/webapp_manifest', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/webmention/webapp_manifest', $data));
        }
    }
