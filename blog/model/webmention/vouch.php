...
class ModelWebmentionVouch extends Model {
    public function recordReferer($referer)
    {
        ...
        $query = $this->db->query(
            "SELECT * " .
            " FROM  " . DB_DATABASE . ".referer_receive_queue " .
        ...
        if (!$query->num_rows == 0) {
            ...
            for ($i = count($domain_parts); $i >= 2; $i--) {
                    ...
                    $query = $this->db->query("SELECT * " .
                        " FROM " . DB_DATABASE . ".untrusted_vouchers " .
            ...
            $this->db->query("INSERT INTO " . DB_DATABASE . ".referer_receive_queue SET url='" . $this->db->escape($referer) . "'");
        }
    }
    ...
    public function processReferers()
    {
        $query = $this->db->query("SELECT id " .
            " FROM " . DB_DATABASE . ".referer_receive_queue rrq JOIN " . DB_DATABASE . ".referrer_ignore ri on rrq.url LIKE ri.url_match ;");

        foreach ($query->rows as $queue_entry) {
            $this->db->query("DELETE FROM " . DB_DATABASE . ".referer_receive_queue where id = " . (int)$queue_entry['id']);
        }
        ...
        while ($entry && !empty($entry)) {
            ...
            if ($valid_link_found) {
                ...
                $query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".vouches WHERE domain = '" . $this->db->escape($domain) . "'");
                ...
                if (!$already_filled) {
                    $this->db->query(
                        ($already_existing ? "UPDATE " : "INSERT INTO ") . DB_DATABASE . ".vouches " .
            ...
            $this->db->query("DELETE FROM " . DB_DATABASE . ".referer_receive_queue where id = " . (int)$entry['id']);

            //get the next queued item
            $query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".referer_receive_queue " .
                " LIMIT 1");
            $entry = $query->row;
        }
    }
    ...
    //check the DB_DATABASE if we have a link back to this site for this URL
    ...
    public function vouchSearch($url)
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".vouches WHERE domain = '" . $this->db->escape($url_domain) . "'");
        $entry = $query->row;
    ...
    public function isWhiteListed($url)
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".whitelist WHERE domain = '" . $this->db->escape($url_domain) . "'");
    ...
    public function getWhitelist($getAll = false)
    {
        ...
        if (!$data) {
            $query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".whitelist " . ($getAll ? "" : "WHERE public=1"));
            $data = $query->rows;
    ...
    public function addWhitelistEntry($url)
    {
        ...
        if ($domain && !empty($domain)) {
            $query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".whitelist WHERE domain='" . $this->db->escape($domain) . "'");
            $found = $query->row;
            if (!$found || empty($found)) {
                $this->db->query("INSERT INTO " . DB_DATABASE . ".whitelist(domain, public) VALUES ('" . $this->db->escape($domain) . "', 0)");
            }
        }
    }
    ...
    {
        $this->db->query("UPDATE " . DB_DATABASE . ".whitelist SET public = 1 WHERE id = " . (int)$id . " LIMIT 1");
        $this->cache->delete('whitelist');
    }
    public function setWhitelistEntryPrivate($id)
    {
        $this->db->query("UPDATE " . DB_DATABASE . ".whitelist SET public = 0 WHERE id = " . (int)$id . " LIMIT 1");
        $this->cache->delete('whitelist');
    }
    public function removeWhitelistEntry($id)
    {
        $this->db->query("DELETE FROM " . DB_DATABASE . ".whitelist WHERE id = " . (int)$id . " LIMIT 1");
        $this->cache->delete('whitelist');
    }
