...
class ModelBlogContext extends Model {
    ...
    public function getImmediateContextForPost($post_id)
    {
        ...
        if (!$data) {
            $query = $this->db->query(
                "SELECT * " .
                " FROM " . DB_DATABASE . ".contexts " .
                " JOIN " . DB_DATABASE . ".context_post on id = context_id " .
        ...
        if (!$data) {
            ...
            while (count($ids) > $prev) {
                ...
                $query = $this->db->query("SELECT parent_id " .
                    " FROM " . DB_DATABASE . ".context_context " .
    ...
    private function getContextId($source_url)
    {
        ...
        if ($page_content !== false) {
            ...
            if (!empty($query->row)) {
                ...
                $this->db->query("INSERT INTO " . DB_DATABASE . ".contexts SET 
                ...
                foreach ($mf2_parsed['items'][0]['properties']['in-reply-to'] as $citation) {
                    if (isset($citation['properties'])) {
                        foreach ($citation['properties']['url'] as $reply_to_url) {
                            ...
                            if ($ctx_id) {
                                $this->db->query("INSERT INTO " . DB_DATABASE . ".context_context SET 
    ...
    public function processContexts()
    {
        $result = $this->db->query("SELECT * " .
            ...
            " JOIN " . DB_DATABASE . ".post_reply_to ON posts.id = post_reply_to.post_id " .
        ...
        while ($post) {
            ...
            $this->db->query(
                "UPDATE " . DB_DATABASE . ".posts SET context_parsed = 1 " .
                " WHERE post_id = " . (int)$post_id
            );
            ...
            if ($context_id) {
                $this->db->query(
                    "INSERT INTO " . DB_DATABASE . ".context_post " .
    ...
    */
    ...
    public function getSyndications($context_id)
    {
        ...
        if (!$data) {
            $query = $this->db->query("SELECT * " .
                " FROM " . DB_DATABASE . ".context_syndication " .
