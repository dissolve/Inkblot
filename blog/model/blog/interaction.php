...
class ModelBlogInteraction extends Model {
    ...
    public function addWebmention($data, $webmention_id, $comment_data, $post_id = null)
    {
        ...
        $query = $this->db->query(
            "SELECT * " .
            " FROM " . DB_DATABASE . ".interaction_syndication " .
        ...
        if ( $post_id || ( isset($data['year']) && isset($data['month']) &&
            ...
            $this->db->query(
                "INSERT INTO " . DB_DATABASE . ".interactions " .
            ...
            $this->db->query(
                "INSERT INTO " . DB_DATABASE . ".interaction_post " .
            ...
            if ( !$syndication_sites ) {
                $syn_site_query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".syndication_sites");
            ...
            if ( isset($comment_data['syndications']) ) {
                foreach ($comment_data['syndications'] as $url) {
                    ...
                    $this->db->query("INSERT INTO " . DB_DATABASE . ".interaction_syndication 
                    ...
                    $query = $this->db->query(
                        "SELECT * " .
                        " FROM " . DB_DATABASE . ".interaction " .
                        " WHERE url='" . $this->db->escape($url) . "' " .
                        " LIMIT 1"
                    );
                    if ( !empty($query->row) ) {
                        $this->db->query(
                            "DELETE FROM " . DB_DATABASE . ".interaction " .
                            " WHERE url='" . $this->db->escape($url) . "' " .
                            " LIMIT 1"
                        );
                    }
    ...
    public function editWebmention($data, $webmention_id, $comment_data, $post_id = null)
    {
        ...
        if ( $query->num_rows ) {
            $this->db->query(
                "UPDATE " . DB_DATABASE . ".interactions " .
                " SET deleted_at = NOW() " .
...
//below this has been upgraded to Interactions methods
    ...
    public function getInteractionsForPost($type, $post_id, $limit = 100, $skip = 0)
    {
        ...
        if ( !$data ) {
            $query = $this->db->query(
                ...
                " FROM " . DB_DATABASE . ".interactions " .
                " JOIN " . DB_DATABASE . ".interaction_post " .
                " ON interactions.id = interaction_post.interaction_id " .
                " JOIN " . DB_DATABASE . ".webmentions " .
    ...
    public function getSyndications($interaction_id)
    {
        ...
        if ( !$data ) {
            $query = $this->db->query(
                "SELECT * " .
                " FROM " . DB_DATABASE . ".interaction_syndication " .
                " JOIN " . DB_DATABASE . ".syndication_sites " .
    ...
    public function addSecondLevelInteraction($interaction_id, $data)
    {
        ...
        $this->db->query(
            "INSERT INTO " . DB_DATABASE . ".interaction_interaction " .
