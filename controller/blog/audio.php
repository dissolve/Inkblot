<?php  
class ControllerBlogAudio extends Controller {
	public function index() {
		...
		$post = $this->model_blog_audio->getAudioByDayCount($year, $month, $day, $daycount);
        ...
        if(intval($post['deleted']) == 1 && !$this->session->data['is_owner']) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/audio.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/audio.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/blog/audio.tpl', $data));
            }
	...
	public function latest() {
		...
		$this->document->setTitle('Latest Audio');
		...
		foreach ($this->model_blog_post->getRecentPostsByType('audio') as $post) {
                $categories = $this->model_blog_category->getCategoriesForPost($post['post_id']);
                $author = $this->model_blog_author->getAuthor($post['author_id']);
                $comment_count = $this->model_blog_post->getCommentCountForPost($post['post_id']);
                $like_count = $this->model_blog_post->getLikeCountForPost($post['post_id']);
