<?php  
class ControllerBlogListen extends Controller {
	public function index() {
        ...
        if(intval($post['deleted']) == 1 && !$this->session->data['is_owner']) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/listen.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/listen.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/blog/listen.tpl', $data));
            }
	...
	public function latest() {
		...
		$this->document->setTitle('Latest Listens');
		$data['title'] = 'Latest Listens';
		...
		foreach ($this->model_blog_post->getRecentPostsByType('listen') as $post) {
            $categories = $this->model_blog_category->getCategoriesForPost($post['post_id']);
            $author = array('link' => $this->url->link('') , 'display_name' => AUTHOR_NAME);
	        $comment_count = $this->model_blog_interaction->getInteractionCountForPost('reply', $post['post_id']);
	        $like_count = $this->model_blog_interaction->getInteractionCountForPost('like', $post['post_id']);
