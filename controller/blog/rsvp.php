...
class ControllerBlogRsvp extends Controller {
	public function index() {
		...
		$post = $this->model_blog_rsvp->getRsvpByDayCount($year, $month, $day, $daycount);
        ...
        if(intval($post['deleted']) == 1 && !$this->session->data['is_owner']) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/rsvp.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/rsvp.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/blog/rsvp.tpl', $data));
            }
	...
	public function latest() {
		...
		$this->document->setTitle('Latest RSVPs');
		$data['title'] = 'Latest RSVPs';
		...
		foreach ($this->model_blog_post->getRecentPostsByType('rsvp') as $post) {
                $categories = $this->model_blog_category->getCategoriesForPost($post['post_id']);
                $author = $this->model_blog_author->getAuthor($post['author_id']);
                $comment_count = $this->model_blog_post->getCommentCountForPost($post['post_id']);
                $like_count = $this->model_blog_post->getLikeCountForPost($post['post_id']);
