<?php  
class ControllerBlogBookmark extends Controller {
	public function index() {
		...
		$post = $this->model_blog_bookmark->getBookmarkByDayCount($year, $month, $day, $daycount);
        ...
        if(intval($post['deleted']) == 1 && !$this->session->data['is_owner']) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/bookmark.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/bookmark.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/blog/bookmark.tpl', $data));
            }
	...
	public function latest() {
		...
		$this->document->setTitle('Latest Bookmarks');
		$data['title'] = 'Latest Bookmarks';
